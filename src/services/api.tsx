...
async fetchBlogPosts(limit: number = 10): Promise<BlogPostFromPayload[]> {
  try {
    ...
    console.log(`Status: ${response.status}, Status Text: ${response.statusText}`); // Melhor depuração
    if (!response.ok) {
      throw new Error(`Erro HTTP: ${response.status}`);
    }
  ...
  formatProductForHome(product: { id: any; name: any; salePrice: any; price: any; images: { image: { url: any; }; }[]; slug: any; category: any; }) {
    return {
      id: product.id,
      name: product.name,
